		...
		Pass
		{
			...
			CGPROGRAM
				...
				uniform float _IOR;
						...
						float2 voronoihash3_g59770( float2 p )
						{
						...
						float voronoi3_g59770( float2 v, float time, inout float2 id, inout float2 mr, float smoothness, inout float2 smoothId )
						{
							float2 n = floor( v );
							float2 f = frac( v );
							float F1 = 8.0;
							...
							for ( j = -2; j <= 2; j++ )
							{
								for ( i = -2; i <= 2; i++ )
							 	{
							 		float2 g = float2( i, j );
							 		float2 o = voronoihash3_g59770( n + g );
				...
				float3 IndirectSpecular1_g59769( float3 Color, float Metallic, float Smoothness, float IOR, float3 Fresnel, float3 WorldPos, float3 WorldNormal, float3 ViewDir )
				{
					// --- 0. PARAMETERS ---
				...
				float3 DirectSpecular( float3 Color, float3 LightmapColor, float Metallic, float Smoothness, float IOR, float3 Fresnel, float3 WorldPos, float3 WorldNormal, float3 ViewDir )
				{
					...
					float3 camDelta = _WorldSpaceCameraPos - WorldPos;
					float playerDistSq = dot(camDelta, camDelta);
					float fadeT = saturate((playerDistSq - fadeStartSq) / max(maxRadiusSq - fadeStartSq, 1e-4));
					...
					for (int i = 0; i < (int)_UdonSpecularLightCount; i++) {
					    ...
					    float3 lDir = diff * rsqrt(distSq + 1e-5);
					    float3 H = normalize(lDir + vDir);
					    // Fresnel Schlick - Punchy Version
					    float f_inv = 1.0 - saturate(dot(H, vDir));
					    float f2 = f_inv * f_inv;
					    float f5 = f2 * f2 * f_inv;
							...
							) : SV_Target
				{
					...
					float time3_g59770 = 0.0;
					float2 voronoiSmoothId3_g59770 = 0;
					float2 coords3_g59770 = IN.ase_texcoord6.xy * _VoronoiScale;
					float2 id3_g59770 = 0;
					float2 uv3_g59770 = 0;
					float voroi3_g59770 = voronoi3_g59770( coords3_g59770, time3_g59770, id3_g59770, uv3_g59770, 0, voronoiSmoothId3_g59770 );
					float saferPower5_g59770 = abs( voroi3_g59770 );
					float2 texCoord4_g59770 = IN.ase_texcoord6.xy * float2( 1,1 ) + float2( 0,0 );
					float simpleNoise8_g59770 = SimpleNoise( texCoord4_g59770*_NoiseScale );
					float saferPower6_g59770 = abs( simpleNoise8_g59770 );
					float clampResult12_g59770 = clamp( ( ( _Intensity * pow( saferPower5_g59770 , 5.0 ) ) * pow( saferPower6_g59770 , _NoisePower ) ) , 0.0 , 1.0 );
					float3 temp_cast_1 = (clampResult12_g59770).xxx;
					...
					float3 appendResult139_g59702 = (float3(sqrt( ( 2.0 / 3.0 ) ) , 0.0 , ( 1.0 / sqrt( 3.0 ) )));
					...
					float3 normalizeResult326_g59702 = normalize( Normal_Map700_g5296 );
					float3 Normal_Map318_g59702 = normalizeResult326_g59702;
					float dotResult121_g59702 = dot( appendResult139_g59702 , Normal_Map318_g59702 );
					float temp_output_2_0_g59704 = saturate( dotResult121_g59702 );
					...
					#endif
					float3 appendResult146_g59702 = (float3(( -1.0 / sqrt( 6.0 ) ) , ( 1.0 / sqrt( 2.0 ) ) , ( 1.0 / sqrt( 3.0 ) )));
					float dotResult122_g59702 = dot( appendResult146_g59702 , Normal_Map318_g59702 );
					float temp_output_2_0_g59703 = saturate( dotResult122_g59702 );
					...
					#endif
					float3 appendResult149_g59702 = (float3(( -1.0 / sqrt( 6.0 ) ) , ( -1.0 / sqrt( 2.0 ) ) , ( 1.0 / sqrt( 3.0 ) )));
					float dotResult120_g59702 = dot( appendResult149_g59702 , Normal_Map318_g59702 );
					float temp_output_2_0_g59705 = saturate( dotResult120_g59702 );
					...
					#endif
					float4 RNM_0926_g5296 = ( ( ( ( ( temp_output_2_0_g59704 * temp_output_2_0_g59704 ) * staticSwitch1061_g5296 ) + ( ( temp_output_2_0_g59703 * temp_output_2_0_g59703 ) * staticSwitch1062_g5296 ) ) + ( ( temp_output_2_0_g59705 * temp_output_2_0_g59705 ) * staticSwitch1063_g5296 ) ) * 1.5 );
					...
					float3 Color1_g59769 = oAlbedo6_g5296;
					float Metallic1_g59769 = Metallic1239_g5296;
					...
					#endif
					float temp_output_19_0_g59734 = staticSwitch1004_g5296;
					...
					float3 tanNormal2_g59734 = World_Normal2508_g5296;
					float3 worldNormal2_g59734 = float3( dot( tanToWorld0, tanNormal2_g59734 ), dot( tanToWorld1, tanNormal2_g59734 ), dot( tanToWorld2, tanNormal2_g59734 ) );
					float3 temp_output_3_0_g59734 = ddx( worldNormal2_g59734 );
					float dotResult5_g59734 = dot( temp_output_3_0_g59734 , temp_output_3_0_g59734 );
					float3 temp_output_4_0_g59734 = ddy( worldNormal2_g59734 );
					float dotResult6_g59734 = dot( temp_output_4_0_g59734 , temp_output_4_0_g59734 );
					#ifdef _USEGEOMETRICAA_ON
					float staticSwitch15_g59734 = min( temp_output_19_0_g59734, ( 1.0 - pow( saturate( max( dotResult5_g59734, dotResult6_g59734 ) ) , 0.25 ) ) );
					#else
					float staticSwitch15_g59734 = temp_output_19_0_g59734;
					#endif
					float Smoothness1399_g5296 = staticSwitch15_g59734;
					float Smoothness1_g59769 = Smoothness1399_g5296;
					float IOR2700_g5296 = _IOR;
					float IOR1_g59769 = IOR2700_g5296;
					float3 temp_cast_5 = (0.04).xxx;
					float3 lerpResult1473_g5296 = lerp( temp_cast_5 , oAlbedo6_g5296 , Metallic1239_g5296);
					...
					float3 Fresnel1_g59769 = Fresnel1560_g5296;
					float3 World_Position2505_g5296 = PositionWS;
					float3 WorldPos1_g59769 = World_Position2505_g5296;
					float3 WorldNormal1_g59769 = World_Normal2508_g5296;
					float3 ViewDir1_g59769 = View_Direction2511_g5296;
					float3 localIndirectSpecular1_g59769 = IndirectSpecular1_g59769( Color1_g59769 , Metallic1_g59769 , Smoothness1_g59769 , IOR1_g59769 , Fresnel1_g59769 , WorldPos1_g59769 , WorldNormal1_g59769 , ViewDir1_g59769 );
					float3 temp_output_2768_0_g5296 = localIndirectSpecular1_g59769;
					float grayscale2713_g5296 = Luminance( Lightmap46_g5296 );
					float smoothstepResult2430_g5296 = smoothstep( 0.0 , 0.05 , grayscale2713_g5296);
					#ifdef _USELIGHTMAPPEDREFLECTIONS_ON
					float3 staticSwitch1469_g5296 = ( temp_output_2768_0_g5296 * smoothstepResult2430_g5296 );
					#else
					float3 staticSwitch1469_g5296 = temp_output_2768_0_g5296;
					#endif
					float3 Reflections1419_g5296 = staticSwitch1469_g5296;
					float3 Color97_g59760 = oAlbedo6_g5296;
					float3 LightmapColor97_g59760 = Lightmap46_g5296;
					float Metallic97_g59760 = Metallic1239_g5296;
					float Smoothness97_g59760 = Smoothness1399_g5296;
					float IOR97_g59760 = IOR2700_g5296;
					float3 Fresnel97_g59760 = Fresnel1560_g5296;
					float3 WorldPos97_g59760 = World_Position2505_g5296;
					float3 WorldNormal97_g59760 = World_Normal2508_g5296;
					float3 ViewDir97_g59760 = View_Direction2511_g5296;
					float3 localDirectSpecular97_g59760 = DirectSpecular( Color97_g59760 , LightmapColor97_g59760 , Metallic97_g59760 , Smoothness97_g59760 , IOR97_g59760 , Fresnel97_g59760 , WorldPos97_g59760 , WorldNormal97_g59760 , ViewDir97_g59760 );
					float3 Speculars2560_g5296 = localDirectSpecular97_g59760;
					#ifdef _LIGHTMAPDEBUG
...
/*ASEBEGIN
Version=19907
Node;AmplifyShaderEditor.FunctionNode, AmplifyShaderEditor, Version=0.0.0.0, Culture=neutral, PublicKeyToken=null;3034;192,-1200;Inherit;False;Meenphie;0;;5296;b3ba55a08dd6b49c7be16c6f35cf2033;6,1008,0,2632,0,2635,0,2636,0,2670,0,2619,0;0;6;FLOAT3;625;FLOAT3;624;FLOAT;156;FLOAT;427;FLOAT;1024;FLOAT3;2765
Node;AmplifyShaderEditor.FunctionNode, AmplifyShaderEditor, Version=0.0.0.0, Culture=neutral, PublicKeyToken=null;3038;224,-976;Inherit;False;Snow Particles;50;;59770;5e2d84d094b538e50a87a98a44c75956;0;0;1;FLOAT;0
