...
Shader "Meenphie/Standard/Winter/Snow"
{
	Properties
	{
		...
		_NoiseIntensity( "Noise Intensity", Float ) = 10
		...
		Pass
		{
			...
			CGPROGRAM
				...
				uniform float4 _EmissionMap_ST;
						...
						float2 voronoihash3_g5293( float2 p )
						{
						...
						float voronoi3_g5293( float2 v, float time, inout float2 id, inout float2 mr, float smoothness, inout float2 smoothId )
						{
							float2 n = floor( v );
							float2 f = frac( v );
							float F1 = 8.0;
							...
							for ( j = -2; j <= 2; j++ )
							{
								for ( i = -2; i <= 2; i++ )
							 	{
							 		float2 g = float2( i, j );
							 		float2 o = voronoihash3_g5293( n + g );
							...
							) : SV_Target
				{
					...
					float time3_g5293 = 0.0;
					float2 voronoiSmoothId3_g5293 = 0;
					float2 coords3_g5293 = IN.ase_texcoord6.xy * _Scale;
					float2 id3_g5293 = 0;
					float2 uv3_g5293 = 0;
					float voroi3_g5293 = voronoi3_g5293( coords3_g5293, time3_g5293, id3_g5293, uv3_g5293, 0, voronoiSmoothId3_g5293 );
					float saferPower5_g5293 = abs( voroi3_g5293 );
					float2 texCoord4_g5293 = IN.ase_texcoord6.xy * float2( 1,1 ) + float2( 0,0 );
					float simpleNoise8_g5293 = SimpleNoise( texCoord4_g5293*_NoiseScale );
					float saferPower6_g5293 = abs( simpleNoise8_g5293 );
					float clampResult12_g5293 = clamp( ( ( _Intensity * pow( saferPower5_g5293 , 5.0 ) ) * pow( saferPower6_g5293 , _NoiseIntensity ) ) , 0.0 , 1.0 );
					float3 temp_cast_1 = (clampResult12_g5293).xxx;
					
					float White38_g5250 = 1.0;
					float4 temp_cast_2 = (White38_g5250).xxxx;
					...
					float4 staticSwitch1014_g5250 = temp_cast_2;
					...
					#else
					float4 staticSwitch1017_g5250 = temp_cast_5;
					#endif
		...
		Pass
		{
			...
			CGPROGRAM
					...
					) : SV_Target
				{
					...
					float time3_g5293 = 0.0;
					float2 voronoiSmoothId3_g5293 = 0;
					float2 coords3_g5293 = IN.ase_texcoord5.xy * _Scale;
					float2 id3_g5293 = 0;
					float2 uv3_g5293 = 0;
					float voroi3_g5293 = voronoi3_g5293( coords3_g5293, time3_g5293, id3_g5293, uv3_g5293, 0, voronoiSmoothId3_g5293 );
					float saferPower5_g5293 = abs( voroi3_g5293 );
					float2 texCoord4_g5293 = IN.ase_texcoord5.xy * float2( 1,1 ) + float2( 0,0 );
					float simpleNoise8_g5293 = SimpleNoise( texCoord4_g5293*_NoiseScale );
					float saferPower6_g5293 = abs( simpleNoise8_g5293 );
					float clampResult12_g5293 = clamp( ( ( _Intensity * pow( saferPower5_g5293 , 5.0 ) ) * pow( saferPower6_g5293 , _NoiseIntensity ) ) , 0.0 , 1.0 );
					float3 temp_cast_1 = (clampResult12_g5293).xxx;
...
Node;AmplifyShaderEditor.FunctionNode, AmplifyShaderEditor, Version=0.0.0.0, Culture=neutral, PublicKeyToken=null;3031;224,-928;Inherit;False;Snow Particles;36;;5293;5e2d84d094b538e50a87a98a44c75956;0;0;1;FLOAT;0
