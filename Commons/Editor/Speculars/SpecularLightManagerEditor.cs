...
public static class SpecularLightManagerEditor
{
    ...
    private static void Rescan(SpecularLightManager mgr)
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            _bakedIntensities[i] = oldBakedIntByLight.TryGetValue(l, out float prevBakedInt) && prevBakedInt > 0f
                ? prevBakedInt : l.intensity;

            _bakedColors[i] = oldBakedColByLight.TryGetValue(l, out Vector3 prevBakedCol)
                ? prevBakedCol : new Vector3(l.color.r, l.color.g, l.color.b);
        }
    ...
    private static void UpdateDebugCounts(int finalCount)
    {
        ...
        SpecularLightManager mgr = _worldRoot != null
            ? _worldRoot.GetComponent<SpecularLightManager>() : null;
        if (mgr == null) return;
    ...
    private static bool UpdateLiveData()
    {
        ...
        for (int li = 0; li < _lights.Length; li++)
        {
            ...
            float bakedInt   = (_bakedIntensities != null && li < _bakedIntensities.Length && _bakedIntensities[li] > 0f)
    ...
    private static void FillUploadBuffers(int finalCount)
    {
        for (int i = 0; i < MAX_LIGHTS; i++)
        {
            if (i < finalCount)
            {
                ...
                float bakedInt   = (_bakedIntensities != null && idx < _bakedIntensities.Length && _bakedIntensities[idx] > 0f)
                    ? _bakedIntensities[idx] : intensity;
                Vector3 bakedCol = (_bakedColors != null && idx < _bakedColors.Length)
                    ? _bakedColors[idx] : rawColor;
                ...
                _shaderRight[i]      = new Vector4(l.transform.right.x, l.transform.right.y, l.transform.right.z, _halfExtents[idx].x);
                _shaderUp[i]         = new Vector4(l.transform.up.x,    l.transform.up.y,    l.transform.up.z,    _halfExtents[idx].y);
                _shaderDir[i]        = new Vector4(l.transform.forward.x, l.transform.forward.y, l.transform.forward.z, cosOuter);
