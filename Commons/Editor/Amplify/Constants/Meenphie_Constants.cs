...
{
public static class Meenphie_CONSTANTS
    {
        public static Color CategoryColor
        {
            get
            {
                ...
                {
                    return Meenphie_CONSTANTS.ColorDarkGray;
                }
                else
                {
                    return Meenphie_CONSTANTS.ColorLightGray;
                }
            }
        }
    ...
    public static class Meenphie_Drawers
    {
        public static bool DrawInspectorCategory(string bannerText, bool enabled, bool colapsable, float top, float down, Material material)
        {
            ...
            EditorGUI.DrawRect(fillRect, Meenphie_CONSTANTS.CategoryColor);
            EditorGUI.DrawRect(lineRect, Meenphie_CONSTANTS.LineColor);

            GUI.color = new Color(1, 1, 1, 0.9f);

            GUI.Label(titleRect, bannerText, Meenphie_CONSTANTS.HeaderStyle);
            ...
            if (material.GetTag("RenderPipeline", false) != "HDRenderPipeline")
            {
                ...
                if (colapsable)
                {
                    if (enabled)
                    {
                        GUI.Label(arrowRect, "<size=10>▼</size>", Meenphie_CONSTANTS.HeaderStyle);
